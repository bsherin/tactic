...
function combinedCompletions(context) {
  let aiRCText = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
  let extraSelfCompletions = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];
  ...
  return {
    from: from,
    to: context.pos,
    options: [...filterCompletions(localCompletions), ...filterCompletions(languageCompletions), ...selfCompletions(context, extraSelfCompletions).options, ...periodCompletions(context).options, ...ai_comp],
    span: true
  };
}
create_api();