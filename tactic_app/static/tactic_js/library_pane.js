    ...
    render() {
        ...
        let menu_items = this.props.items.map((item, index) => {
            if (item.text == "__divider__") {
                ...
                return React.createElement(MenuItem, { icon: item.icon, disabled: disabled,
        ...
        return React.createElement(
            Menu,
            null,
            React.createElement(Menu.Divider, { title: this.props.selected_rows[0].name, className: "context-menu-header" }),
            menu_items
        );
    }
}
...
class LibraryPane extends React.Component {
    ...
    _handleMetadataChange(changed_state_elements) {
        if (!this.props.multi_select) {
            ...
            if (Object.keys(changed_state_elements).includes("tags")) {
                ...
                this._updatePaneState({ selected_resource: revised_selected_resource }, this._saveFromSelectedResource);
            } else {
            ...
            revised_selected_resource = Object.assign(revised_selected_resource, changed_state_elements);
            revised_selected_resource["tags"] = revised_selected_resource["tags"].join(" ");
            this._updatePaneState({ selected_resource: revised_selected_resource }, this._overwriteCommonTags);
        }
    }
    ...
    addOneTag(res_name, the_tag) {
        ...
        let self = this;
        postAjaxPromise("save_metadata", result_dict).then(function () {
    ...
    _doTagDelete(tag) {
        ...
        let self = this;
        postAjaxPromise("delete_tag", result_dict).then(function (data) {
            self._refresh_func();
        }).catch(doFlash);
    }
    ...
    _doTagRename(tag_changes) {
        ...
        let self = this;
        postAjaxPromise("rename_tag", result_dict).then(function (data) {
            self._refresh_func();
        }).catch(doFlash);
    }
    ...
    _duplicate_func(duplicate_view, resource_name = null) {
        ...
        $.getJSON($SCRIPT_ROOT + "get_resource_names/" + res_type, function (data) {
            showModalReact(`Duplicate ${res_type}`, "New Name", DuplicateResource, res_name, data.resource_names);
        });
        let self = this;
    ...
    _repository_copy_func() {
        ...
        function ImportResource(new_name) {
            ...
            postAjaxPromise("/copy_from_repository", result_dict).then(doFlash).catch(doFlash);
        }
    ...
    _send_repository_func() {
        ...
        function ShareResource(new_name) {
            ...
            postAjaxPromise('/send_to_repository', result_dict).then(doFlash).catch(doFlash);
        }
        return res_name;
    }
    ...
    render() {
        ...
        if (this.props.aux_pane == null) {
            ...
            right_pane = React.createElement(
                React.Fragment,
                null,
                mdata_element,
                React.createElement(
                    "div",
                    { className: "d-flex flex-row justify-content-around", style: { marginTop: 20 } },
                    React.createElement(
                        Button,
                        { fill: false,
                            small: true,
                ...
                React.createElement(
                    Collapse,
                    { isOpen: this.state.auxIsOpen, keepChildrenMounted: true },
                    this.props.aux_pane
                )
            );
        }
        ...
        let left_pane = React.createElement(
            ...
            React.createElement(
                "div",
                { className: "d-flex flex-row", style: { "maxHeight": "100%" } },
                React.createElement(
                    "div",
                    { className: "d-flex justify-content-around", style: { paddingRight: 10 } },
                    React.createElement(TagButtonList, _extends({ res_type: this.props.res_type,
