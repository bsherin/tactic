...
class ConsoleComponent extends React.Component {
    ...
    _resetConsole() {
        ...
        this.props.setConsoleFieldValue("console_items", new_console_items);
        postWithCallback(window.main_id, "clear_console_namespace", {})
    }

    _clearConsole() {
        this.props.setConsoleFieldValue("console_items", [])
    }

    _toggleConsoleLog() {
        let self = this;
        if (this.props.show_error_log) {
            this.props.setConsoleFieldValue("show_console_error_log", false)
        }
        else {
            if (self.pseudo_tile_id == null) {
                postWithCallback(window.main_id, "get_pseudo_tile_id", {}, function (res) {
                    ...
                    if (self.pseudo_tile_id == null) {
                        self.props.setConsoleFieldValue("console_error_log_text", "pseudo-tile is initializing...", ()=>{
                            self.props.setConsoleFieldValue("show_console_error_log", true)
                        });
                    }
                    else{
                        postWithCallback("host", "get_container_log", {"container_id": self.pseudo_tile_id}, function (res) {
                            ...
                            self.props.setConsoleFieldValue("console_error_log_text", log_text, ()=>{
                                self.props.setConsoleFieldValue("show_console_error_log", true)
                            });
                        })
                    }
                ...
                postWithCallback("host", "get_container_log", {"container_id": self.pseudo_tile_id}, function (res) {
                    self.props.setConsoleFieldValue("console_error_log_text", res.log_text, () => {
                            self.props.setConsoleFieldValue("show_console_error_log", true)
                        }
                    );
                })
    ...
    _setFocusedItem(unique_id) {
        this.props.setConsoleFieldValue("console_item_with_focus", unique_id)
    }

    _zoomConsole () {
        this.props.setConsoleFieldValue("console_is_zoomed", true)
    }

    _unzoomConsole () {
        this.props.setConsoleFieldValue("console_is_zoomed", false)
    }
