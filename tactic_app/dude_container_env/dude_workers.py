...
from dude_users import load_user, ModuleNotFoundError
import gevent
...
import common_data
from common_data import app, socketio, use_ssl, db
from library_views import tile_manager, project_manager, collection_manager, list_manager, code_manager
import uuid
import sys
import copy
import datetime
...
global_tile_manager = common_data.global_tile_manager

from js_source_management import _develop


class DudeWorker(QWorker):
    def __init__(self):
    ...
    def update_code_task(self, data_dict, task_packet):
        try:
            ...
            self.post_task(common_data.test_tile_container_id, "clear_and_load_code",
    ...
    def load_tile_module_task(self, data, task_packet):
        def loaded_source(res_dict):
            try:
                print("in loaded_source handling response")
                ...
                if "is_default" in data:
                    ...
                    is_default = False
                print("about to add one user_tile_module")
                global_tile_manager.add_user_tile_module(user_obj.username,
                                                         ...
                                                         tile_module_name,
                                                         is_default)
                print("about to emit_loaded_tile_update")
                self.emit_loaded_tile_update(user_obj)
                ...
                print('checkin callback type')
                if not task_packet["callback_type"] == "no_callback":
                    print("submiting_response")
                    self.submit_response(task_packet, {"success": True, "message": "Tile module successfully loaded",
                                                       "alert_type": "alert-success"})
                return
            except Exception as ex2:
                msg = self.get_short_exception_dict(ex2, "Error in loaded_source")
                print(msg)
                if not task_packet["callback_type"] == "no_callback":
                    self.submit_response(task_packet, msg)
                return
        ...
        try:
            ...
            try:
                ...
                self.post_task(common_data.test_tile_container_id, "load_source",
    ...
    def remove_mainwindow_task(self, data):
        ...
        client_worker.remove_from_heartbeat_table(main_id)
        return {"success": True}
    ...
    def emit_table_message(self, data):
        from common_data import socketio
    ...
    def print_to_console(self, data):
        from common_data import app, socketio
        user_id = data["user_id"]
        user_obj = load_user(user_id)
            ...
            try:
                ...
                try:
                    if "empty" not in task_packet:
                        print("got non-empty clien task_packet")
                        if task_packet["callback_id"] is not None:
                            print("it has a callback")
...
common_data.dude_worker = DudeWorker()
common_data.client_worker = ClientWorker()
common_data.client_worker.my_id = "client_" + common_data.dude_worker.my_id
common_data.dude_worker.start()
common_data.client_worker.start()
