...
from exception_mixin import TileModuleNotFoundError, TileMetadataNotFoundError
...
class TileTasksMixin:
    ...
    def grab_processed_tile_metadata_task(self, data):
        ...
        if result is None:
            raise TileMetadataNotFoundError(f"Metadata for tile module {tile_module_name} not found.")
        result["success"] = True
        return result
    ...
    def load_tile_module_task(self, data, task_packet):
        ...
        def loaded_source(res_dict):
            if not res_dict["success"]:
                log.warning("load_source failed", tile_module_name=tile_module_name, message=res_dict["message"])
        ...
        try:
            ...
            log.exception("Error loading tile")
            if not task_packet["callback_type"] == "no_callback":
