...
class ObjectAPIMixin:
    ...
    @staticmethod
    def new_collection(doc_type, doc_dict=None):
        return DetachedTacticCollection(doc_type, doc_dict)

    @staticmethod
    def create_row(row_dict=None):
        if row_dict is None:
            row_dict = {}
    ...
    @staticmethod
    def create_document(doc_data=None, docname="document1", metadata=None):
        return DetachedTacticDocument(doc_data, docname, metadata)

    @staticmethod
    def create_freeform_document(docname="document1", lines=None, metadata=None):
        return DetachedFreeformTacticDocument(docname, lines, metadata)

    @staticmethod
    def create_collection_object(doc_type, doc_list=None):
