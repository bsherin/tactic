...
try:
    ...
    log.exception("*** fatal error during imports in tile_main ***")
    log.critical("*** exiting tile_main due to fatal error ***")
    raise
...
class KillWorker(QWorker):
    ...
    def handle_delivery(self, channel, method, props, body):
        task_id = new_task_id()
        with bind_request(task_id, "received", "kill_task"):
            ...
            try:
                ...
                if task_packet["task_type"] == "restart":
                    os.execv(sys.executable, [sys.executable, "-u", "-m", "tile_container_env.tile_main"])

            except Exception:
                log.exception("Error in kill handle delivery")
            return
...
class HeartbeatGenerator:
    ...
    def post_heartbeat(self):
        new_id = new_task_id()
        with bind_request(new_id, "presend", "post_heartbeat"):
            ...
            try:
                ...
                log.exception("Error posting heartbeat")
            return
...
class TileWorker(QWorker):
    ...
    def post_to_main(self, task_type, task_data=None, callback_func=None,
        ...
        self.post_task("main_service", task_type, task_data, callback_func,
                       callback_data, expiration, special_reply_to)
    ...
    @task_worthy
    def get_options(self, _data_dict):
    ...
    def recreate_from_save(self, data):
        try:
            ...
            log.exception("Error in tile_main recreate from save")
    ...
    @staticmethod
    def extract_option_names(opt_dict):
        opt_names = []
        for opt in opt_dict:
    ...
    @task_worthy
    def kill_me(self, _data):
        try:
            self.connection.close()
            kill_worker.connection.close()
        except:
            log.exception("got error in kill_me when closing connections. exiting anyway")
        sys.exit()

    @task_worthy
    def get_reload_info(self, _data):
        return {"success": True,
                "reload_dict": self.get_reload_dict()}
